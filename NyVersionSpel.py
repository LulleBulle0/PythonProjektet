...
class Monster:
    # Ändra namn på monster, lägga till flera?
    monster_types = [
        ("Lilltrollet", 40), 
        ("Jätten", 50), 
        ("Lilleskutt", 60)
...
class Game:
    ...
    def monster_event(self):
        ...
        while self.player.HP > 0 and monster.strength > 0:
            ...
            if choice == "1":
                # Attackera
                player_damage = rand.randint(10, 25) # kan ändra hur mycket damage man gör beroende på level om vi hinner.
                ...
                # Gör så att item sköld ökar mängden damage som blockas
            ...
            # Kontrollera om spelaren dött
            if self.player.HP <= 0:
                print(f"\n {monster.name} besegrade dig! Du förlorade spelet.")
                break

    # Lägg till valet om man vill eller inte vill ha innehåll av kista
